...
client.on("message", message => {
  if (message.content === prefix + "bot") {
    const bot = new Discord.RichEmbed()
      ...
      .addField("**Bot Owner** :  ", `» <@606864172755714078>`, true) // تعديل اساسي غير الايدي لايدي حسابك
      .setImage("")
...
\`${prefix}invites\` : عدد الدعوات للسيرفر
...
\`${prefix}gcreate\` : لعمل قيف اوي 
\`${prefix}bc\` : لارسال لجميع الاعضاء
\`${prefix}obc\` : لارسال للونلاين
...
\`${prefix}np\` : عرض مايتم تشغيله الان [np] 
\`${prefix}repeat\` : تكرار الاغنية 
\`دسكورد صاحب البوت \` : <https://discord.gg/SYb24yJ>
...
client.on("message", message => {
  ...
  /*
  if (message.content.startsWith(prefix + "حماية")) {
    if (message.author.id == message.guild.ownerID)
      message.author.send
...
client.on("message" , message => {
  ...
  if(!message.member.hasPermissions("ADMINISTRATOR")) return message.reply("**ليس لديك الصلاحيات الكافية**")
    message.guild.members.filter(o => o.presence.status !== "offline").forEach(o => {
    o.send(`${argss}\n ${o}`)
    })
    message.channel.send(`\`${message.guild.members.filter(o => o.presence.status !== "offline").size}\` : عدد الأعضاء المستلمين`)
    .then((message => {message.delete(3000)})
          )
  }
})
...
const getYoutubeID = require('get-youtube-id')
const fetchV'error', c


client.on('ready', ()
    console.log(`Created By: Alone`);
...
const command = args.shift().toLowerCase();
    ...
    if(cmd === 'play') {
        const voiceChannel = msg.member.voiceChannel;
        if(!voiceChannel) return msg.channel.send(`:no_entry_sign: يجب عليك الدخول الى روم صوتي أولاً`);
        const permissions = voiceChannel.permissionsFor(msg.client.user);
        if(!permissions.has('CONNECT')) {
            return msg.channel.send(`:no_entry_sign: ليس لدي الصلاحيات الكفايه لدخول هذا الروم`);
        }

        if(!permissions.has('SPEAK')) {
            return msg.channel.send(`:no_entry_sign: ليس لدي الصلاحيات الكفايه لاصدار الصوت`);
        }
	...
	if (!serverQueue) {
      ...
      repeating: false
		...
		try {
			...
			console.error(`I could not join the voice channel: ${error}`);
			active.delete(msg.guild.id);
			return msg.channel.send(`لم استطع الدخول الى هذا الروم`);
		}
	} else {
		serverQueue.songs.push(song);
      ...
      setTimeout(() => {//:watch: Loading... [let]
        m.edit(`:notes: تمت اضافة **${song.title}**` + '(` ' + song.duration + ')`' + ` الى قائمة التشغيل ` + `${serverQueue.songs.length}`);
      }, 500)
    }) 
	}
	return undefined;
...
function play(guild, song) {
  ...
  if(serverQueue.repeating) {
	...
	serverQueue.textChannel.send(':notes: تمت اضافة **' + song.title + '** (`' + song.duration + '`) ');
}
	...
	dispatcher.setVolumeLogarithmic(serverQueue.volume / 100);
        ...
        if(msg.guild.me.voiceChannel !== msg.member.voiceChannel) return msg.channel.send(`يجب عليك الدخول الى روم ${msg.guild.me.voiceChannel.name}`)
        if(!msg.member.hasPermission('ADMINISTRATOR')) {
          msg.react('❌')
          return msg.channel.send('ليس لديك الصلاحيات الكافيه');
        }
        let queue = active.get(msg.guild.id);
        if(queue.repeating) return msg.channel.send('وضع التكرار يعمل ، لايمكنك تخطي الموسيقى ، من فضلك الغي وضع التكرار أولا')
        queue.songs = [];
        queue.connection.dispatcher.end();
        return msg.channel.send(':notes: تم ايقاف البوت ومسح قائمة التشغيل');
      ...
      let queue = active.get(msg.guild.id);
        ...
        if(!vCh) return msg.channel.send('لا يمكنك التحطي ، لانك لست في روم صوتي');

        if(!queue) return msg.channel.send('لا يوجد موسيقى لتخطيها');

        if(queue.repeating) return msg.channel.send('لا يمكنك التخطي بسبب وضع التكرار');
        ...
        if(queue.votes.includes(msg.member.id)) return msg.say(`لقد صوتت بالفعل للتخطي ${queue.votes.length}/${req}`);
        ...
        msg.channel.send(`**تم التصويت للتخطي ${queue.votes.length}/${req}**`)
      ...
      let queue = active.get(msg.guild.id);
        ...
        if(!vCh || vCh !== msg.guild.me.voiceChannel) return msg.channel.send(`تعذر الايقاف المؤقت`);

        if(!queue) {
            return msg.channel.send('لا توجد موسيقى للايقاف المؤقت')
        }

        if(!queue.playing) return msg.channel.send(':no_entry_sign: لا يوجد موسيقى لايقافها')
        ...
        msg.channel.send(':notes: تمت العملية ' + args + '. **اكتب** `' + prefix + 'resume` للاكمال!')
      ...
      let queue = active.get(msg.guild.id);
        ...
        if(!vCh || vCh !== msg.guild.me.voiceChannel) return msg.channel.send(`انت لست في نفس الروم الصوتي`);

        if(!queue) return msg.channel.send(':notes: لا يوجد موسيقى موقوفه لاكمالها')

        if(queue.playing) return msg.channel.send(':notes: لا يوجد موسيقى موقوفه لاكمالها')
        ...
        msg.channel.send(':notes: تمت العملية')

    } else if(cmd === 'volume') {

      let queue = active.get(msg.guild.id);

      if(!queue || !queue.songs) return msg.channel.send(':notes: لا يوجد موسيقى لتغيير الصوت');
      ...
      if(isNaN(args[0])) return msg.channel.send(':notes: يسمح بأرقام فقط');

      if(parseInt(args[0]) > 100) return msg.channel.send('You can\'t set the volume more than 100.')
//:speaker: Volume changed from 20 to 20 ! The volume has been changed from ${queue.volume} to ${args[0]}
      msg.channel.send(':speaker: تم تغيير الصوت من (`' + queue.volume + '`) الى (`' + args[0] + '`)');
      ...
      if(!queue) return msg.channel.send(':no_entry_sign: يجب ان يكون هناك موسيقى لاستخدام هذا الامر');
      ...
      if(!vCh || vCh !== msg.guild.me.voiceChannel) return msg.channel.send('انت لست في نفس الروم الصوتي التي فيها');

      let queue = active.get(msg.guild.id);

      if(!queue || !queue.songs) return msg.channel.send('لا يوجد موسيقى لتكرارها');

      if(queue.repeating) {
        queue.repeating = false;
        return msg.channel.send(':arrows_counterclockwise: **وضع التكرار** (`مفعل`)');
      } else {
        queue.repeating = true;
        return msg.channel.send(':arrows_counterclockwise: **وضع التكرار** (`معطل`)');
      }
      ...
      if(!vCh || vCh !== msg.guild.me.voiceChannel) return msg.channel.send('انت لست في نفس الروم التي فيها');
      ...
      if(queue.repeating) {
        ...
        msg.channel.send('التخطي المباشر, وضع التكرار يعمل')

        queue.connection.dispatcher.end('التخطي المباشر ..')
...
client.aliases = new Discord.Collection();
  ...
  skip: { cmd: 'skip', a: ['s','تخطي','next']},
  stop: { cmd: 'stop', a:['ايقاف','توقف'] },
  pause: { cmd: 'pause', a:['pe','لحظة','مؤقت'] },
  resume: { cmd: 'resume', a: ['r','اكمل','استكمال'] },
  volume: { cmd: 'volume', a: ['v','صوت'] },
  queue: { cmd: 'queue', a: ['q','list','قائمة'] },
  repeat: { cmd: 'loop', a: ['l','تكرار','اعادة'] },
  forceskip: { cmd: 'forceskip', a: ['fs', 'fskip'] },
  skipto: { cmd: 'skipto', a: ['st','تخطي الي'] },
  nowplaying: { cmd: 'Nowplaying', a: ['np','الان'] }
};
...
function play(guild, song) {
  ...
  if(serverQueue.repeating) {
    console.log('Repeating');
     ...
     serverQueue.textChannel.send(' تم إضافة ' + song.title +'الى  قائمة الإنتظار('+ song.duration +')! :notes:');
}
	const dispatcher = serverQueue.connection.playStream(ytdl(song.url))
		.on('end', reason => {
			//if (reason === 'Stream is not generating quickly enough.') console.log('Song ended.');
...
let g = client.guilds.get("746850003997294642"); // id server
let c = g.channels.get("74746850004991213653");// id channel
if(c.type === 'voice') {
c.join();
setInterval(() => {
...
client.on("ready", () => {
  ...
  console.log(`${client.users.size} Members`);
   console.log(`${client.channels.size} Channels`);
  console.log(`[ ${client.guilds.map(g => g.name).join(", \n ")} ]`);
  client.user.setActivity(`${prefix}help | K-Music(BETA)`, { type: "WATCHING" });
});
...
- ${prefix}limitchannelDelete الحد الاقصى لحذف الرومات

- ${prefix}antibots on لمنع دخول لبوتات 

- ${prefix}antibots off لايقاف منع دخول البوتات 
